...
const Admin = ()=>{
    ...
    useEffect(() => {
        ...
        if (startInput && !startInput.dataset.initialized) {
          startPicker = new Datepicker(startInput, { autohide: true });
          startInput.dataset.initialized = true;
        }
        if (endInput && !endInput.dataset.initialized) {
          endPicker = new Datepicker(endInput, { autohide: true });
          endInput.dataset.initialized = true;
        }
      
        return () => {
          // Remove leftover elements if necessary
          document.querySelectorAll('.datepicker').forEach(el => el.remove());
        };
      }, []);
    ...
    return(
        ...
        <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-5 mt-4">
            ...
            <div className="relative">
                ...
                <input id="datepicker-range-start" name="startdate" value={form.startdate} onChange={handleChange} type="text" className="bg-gray-150 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-200 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date start"/>
            </div>
